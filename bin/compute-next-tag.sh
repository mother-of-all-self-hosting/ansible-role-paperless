...
#
# Tags look like `v<Paperless-ngx version>-<release>`:
#
# - if defaults/main.yml points at a Paperless-ngx version that has never been
#   released, the release counter restarts at 0 (`v3.0.6-0`)
# - otherwise the counter is incremented (`v3.0.6-1`), but only if something
...
version="$(sed -nE 's|^paperless_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Paperless-ngx version from $defaults_path"
	exit 1
fi
